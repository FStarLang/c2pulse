module Test_union_nested_owns_with_functional_spec_maybe_freeable
...
stru_ok foo s f
ensures 
exists* (t:either uint32 bool). stru_ok foo t f **
pure (match s with
| Inl _ -> t == Inl 1ul
...
stru_ok foo (Inl #uint32 s) f
ensures 
exists* v. stru_ok foo v f ** pure (v == Inl 1ul)
{
let mut foo : (ref stru) = foo;
tag_relation_lemma _; stru_explode (!foo);
ab_is_a (! (! foo)).payload; ab_explode (! (!foo)).payload; elim_stru_payload_a _;
...
(#f:erased _Bool)
requires 
stru_ok foo (Inr #_ #bool s) f
ensures 
stru_ok foo (Inr (not s)) f
{
let mut foo : (ref stru) = foo;
tag_relation_lemma _; stru_explode (!foo);
ab_is_b (! (! foo)).payload; ab_explode (! (!foo)).payload; elim_stru_payload_b _;
...
stru_ok foo (Inr #_ #bool s) f
ensures 
exists* v. stru_ok foo v f ** pure (v == Inr (not s))
{
let mut foo : (ref stru) = foo;
tag_relation_lemma _; stru_explode (!foo);
ab_is_b (! (! foo)).payload; ab_explode (! (!foo)).payload; elim_stru_payload_b _;
...
returns x:ref stru
ensures 
exists* v. stru_ok x v _true_ ** pure (v == Inl 0ul) ** freeable x
{
let mut a : (ref UInt32.t) = alloc_ref #UInt32.t ();
intro_maybe_freeable _ (!a);
...
Mkstru?.payload (! (! x)) := (! ab);
intro_stru_payload_a (Case_ab_a (!a)); ab_recover (!(!x)).payload #(Case_ab_a _); stru_recover (!x); intro_stru_ok (!x);
(! x);
}
