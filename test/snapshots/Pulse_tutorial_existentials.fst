...
(x : ( ref Int32.t) )
requires 
exists* s. (x |-> s) ** pure (fits ( * ) 2 (as_int s))
ensures 
exists* s. (x |-> s) ** pure (as_int s % 2 == 0)
{
...
(x : ( ref Int32.t) )
requires 
exists* w0. (x |-> w0) ** pure (fits ( * ) 2 (as_int w0))
ensures 
...
(y : ( ref Int32.t) )
requires 
exists* wx wy. (x |-> wx) ** (y |-> wy) ** pure (as_int wx % 2 == as_int wy % 2) ** pure (fits (+) (as_int wx) (as_int wy))
ensures 
exists* wx wy. (x |-> wx) ** (y |-> wy)
{
let mut x : (ref Int32.t) = x;
...
x |-> v
requires 
pure (fits (+) (as_int v) (as_int v))
ensures 
exists* w. (x |-> w) ** pure (as_int w % 2 == 0)
{
let mut x : (ref Int32.t) = x;
(make_even (! x));
}
