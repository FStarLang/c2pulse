...
_requires((_slprop) _inline_pulse(stru_ok foo s f))
_ensures((_slprop) _inline_pulse(exists* (t:either uint32 bool). stru_ok foo t f **
        pure (match s with
              | Inl _ -> t == Inl 1ul
...
_requires((_slprop) _inline_pulse(stru_ok foo s f))
_ensures((_slprop) _inline_pulse(stru_ok foo (Inl 1ul) f))
...
_requires((_slprop) _inline_pulse(stru_ok foo (Inl #uint32 s) f))
_ensures((_slprop) _inline_pulse(stru_ok foo (Inl 1ul) f))
...
_requires((_slprop) _inline_pulse(stru_ok foo (Inl #uint32 s) f))
_ensures((_slprop) _inline_pulse(exists* v. stru_ok foo v f ** pure (v == Inl 1ul)))
...
_requires((_slprop) _inline_pulse(stru_ok foo (Inr #_ #bool s) f))
_ensures((_slprop) _inline_pulse(stru_ok foo (Inr (not s)) f))
...
_requires((_slprop) _inline_pulse(stru_ok foo (Inr #_ #bool s) f))
_ensures((_slprop) _inline_pulse(exists* v. stru_ok foo v f ** pure (v == Inr (not s))))
...
_ensures((_slprop) _inline_pulse(exists* v. stru_ok x v _true_ ** pure (v == Inl 0ul) ** freeable x))
stru* mk_union_a ()
{
  unsigned int* a = (unsigned int*)malloc(sizeof(unsigned int));
  _assert((_slprop) _inline_pulse(intro_maybe_freeable _ (!a)));
  *a = 0ul;
  union ab ab;
