...
C2PULSE_BUILD_TYPE=${C2PULSE_BUILD_TYPE:-Debug}
...
cmake -G Ninja -DCMAKE_BUILD_TYPE=$C2PULSE_BUILD_TYPE -DLLVM_ENABLE_ASSERTIONS=ON -DCMAKE_BUILD_WITH_INSTALL_RPATH=ON -DLLVM_PARALLEL_LINK_JOBS=2 -DLLVM_BUILD_LLVM_DYLIB=ON -DLLVM_LINK_LLVM_DYLIB=ON -DLLVM_ENABLE_RTTI=ON -DLLVM_ENABLE_LLD=ON  -DLLVM_ENABLE_PROJECTS="clang;clang-tools-extra" -DLLVM_TARGETS_TO_BUILD="X86" ../llvm
popd
