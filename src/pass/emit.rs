                ...
                Some(ty) => {
                    ...
                    match &ty.val {
                        TypeT::TypeRef(TypeRefKind::Struct(struct_name)) => unaryfn(
                            ...
                            unaryfn(Doc::text("!"), emit_lvalue(env, x)),
                        ),
...
fn emit_structdefn(env: &Env, StructDefn { name, fields }: &StructDefn) -> Doc {
    ...
    let raw_pts_to_name = Doc::text(format!("struct_{}__raw_pts_to", name));

    let mut ses = vec![];
    ses.push(mk_assume_val(
        Doc::text(struct_type_name.clone()),
        &[],
        Doc::text("Type0"),
    ));

    let raw_spec_t = Doc::text(format!("struct_{}__raw_spec_t", name));
    let raw_spec_fld = |fld: &Ident| Doc::text(format!("struct_{}__raw_spec_{}", name, fld));

    ses.push(
        Doc::text("[@@erasable] noeq type")
            .append(Doc::line())
            .append(raw_spec_t.clone())
            .append(Doc::line())
            .append("=")
            .append(Doc::line())
            .append("{")
        ...
        Doc::text("[@@pulse_intro]")
            ...
            .append(mk_assume_val(
                ...
                naryfn([
                    ...
                    Doc::text("emp_inames"),
                    // pre
                    naryfn([raw_pts_to_name.clone(), Doc::text("x"), Doc::text("vx")]),
                    {
                        ...
                        for (fld, _) in fields {
                            post.push(naryfn([
                                ...
                                Doc::text("vx.").append(raw_spec_fld(fld)),
                            ]));
                        }
